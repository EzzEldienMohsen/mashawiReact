...
const NavBarButtons: React.FC = () => {
  ...
  return (
    <div
      className={`gap-x-3 hidden lg:flex  justify-between items-center lg:w-[13%] 2xl:w-[13%]`}
    >
      <CartDrawer isRed={isRed} />

      <button
        onClick={toggleLang}
        className={`rounded-badge text-lg ${
          isRed ? 'text-newRed' : 'text-black'
        } font-tajawal 2xl:rounded-full aspect-square flex items-center justify-center w-10 2xl:w-14 bg-white cursor-pointer`}
      >
        {isLangArabic ? 'EN' : 'AR'}
      </button>
      {token ? (
